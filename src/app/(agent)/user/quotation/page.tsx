...
export default function QuotationJourney() {
  ...
  const [ocrResult, setOcrResult] = useState<any>(null); // เก็บผลดิบจาก AI
  const [showOcrReview, setShowOcrReview] = useState(false); // ควบคุมการแสดงหน้า Review
  ...
  const handleOCR = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      if (result.success) {
        ...
        setShowOcrReview(true); // เปิดหน้า Review
      }
    } catch (err) { alert('AI Scan Failed'); }
    finally { setOcrLoading(false); }
  };

  // ยืนยันเอาข้อมูลจาก OCR เข้าฟอร์ม
  const applyOcrData = () => {
    setFormData(prev => ({
      ...prev,
      vehicle: {
    ...
    try {
      ...
      const res = await fetch('/api/products/contracts', {
        ...
        body: JSON.stringify({
          ...
          plan: selectedPlan,
          vehicle: { ...selectedVehicle, registrationNumber: formData.vehicle.registrationNumber, vin: formData.vehicle.vin, engineNumber: formData.vehicle.engineNumber },
  ...
  return (
    <main style={{ padding: '20px' }}>
      <header style={{ marginBottom: '2rem' }}>
        <h1 style={{ fontSize: '1.5rem', fontWeight: 'bold' }}>ออกกรมธรรม์ (UAT & AI Scan)</h1>
        <p style={{ color: '#666' }}>ระบบตรวจสอบข้อมูลอัตโนมัติด้วย AI 2.5 Flash</p>
      </header>

      {/* Step Indicator */}
      <div style={{ display: 'flex', gap: '1rem', marginBottom: '2rem' }}>
        {['1. ผู้เอาประกัน', '2. ข้อมูลรถ & ที่อยู่', '3. การเงิน & ยืนยัน', '4. สำเร็จ'].map((l, i) => (
      ...
      {/* OCR Review Panel (Conditional Overlay) */}
      {showOcrReview && (
        <div style={{ position: 'fixed', top: 0, left: 0, right: 0, bottom: 0, background: 'rgba(0,0,0,0.7)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 1000, padding: '20px' }}>
          <div className="card" style={{ maxWidth: '800px', width: '100%', background: 'white', borderRadius: '20px', padding: '2rem' }}>
            <h2 style={{ fontSize: '1.25rem', marginBottom: '1.5rem', display: 'flex', alignItems: 'center', gap: '10px' }}>🤖 ตรวจสอบผลการสแกนด้วย AI</h2>
            <p style={{ color: '#666', fontSize: '0.9rem', marginBottom: '1.5rem' }}>กรุณาตรวจสอบและเปรียบเทียบข้อมูลที่ AI อ่านได้จากรูปภาพ หากไม่ถูกต้องสามารถแก้ไขได้ในภายหลัง</p>
            
            <table style={{ width: '100%', borderCollapse: 'collapse', marginBottom: '2rem' }}>
              <thead style={{ background: '#f8f9fa' }}>
                <tr>
                  <th style={{ padding: '12px', textAlign: 'left', fontSize: '0.8rem', color: '#888' }}>หัวข้อ</th>
                  <th style={{ padding: '12px', textAlign: 'left', fontSize: '0.8rem', color: '#888' }}>ข้อมูลจากระบบ</th>
                  <th style={{ padding: '12px', textAlign: 'left', fontSize: '0.8rem', color: '#888' }}>AI อ่านได้ (OCR)</th>
                  <th style={{ padding: '12px', textAlign: 'center', fontSize: '0.8rem', color: '#888' }}>สถานะ</th>
                </tr>
              </thead>
              <tbody>
                {[
                  ...
                  { label: 'เลขทะเบียน', system: '-', ai: ocrResult.registrationNumber },
                  { label: 'เลขตัวถัง (VIN)', system: '-', ai: ocrResult.vin },
                  { label: 'เลขเครื่อง', system: '-', ai: ocrResult.engineNumber },
                ].map((item, idx) => {
                  const isMatch = item.system === '-' || String(item.system).toUpperCase() === String(item.ai).toUpperCase();
                  return (
                    <tr key={idx} style={{ borderBottom: '1px solid #eee' }}>
                      <td style={{ padding: '12px', fontWeight: 'bold', fontSize: '0.9rem' }}>{item.label}</td>
                      <td style={{ padding: '12px', fontSize: '0.9rem' }}>{item.system}</td>
                      <td style={{ padding: '12px', fontSize: '0.9rem', color: isMatch ? '#2ecc71' : '#ff4d4f', fontWeight: 'bold' }}>{item.ai || 'ไม่พบ'}</td>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 350px', gap: '2rem', alignItems: 'start' }}>
        <section className="card" style={{ padding: '2rem', borderRadius: '16px', background: 'white', border: '1px solid #eee' }}>
          ...
          {step === 2 && (
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '1.5rem' }}>
              ...
              <button onClick={() => setStep(3)} className="btn-primary" style={{ gridColumn: 'span 2', marginTop: '1rem', borderRadius: '50px' }}>ตรวจสอบข้อมูล</button>
            </div>
          )}

          {step === 3 && (
            <div style={{ display: 'grid', gap: '1.5rem' }}>
              <h3 style={{ fontSize: '1.1rem' }}>📋 ตรวจสอบ & ชำระเงิน</h3>
              <div style={{ background: '#fff9db', padding: '1rem', borderRadius: '12px', border: '1px solid #ffe066' }}>
                <p style={{ fontSize: '0.85rem' }}>⚠️ ยืนยันข้อมูลรุ่นรถ <strong>{selectedVehicle?.modelName}</strong> และปี <strong>{selectedVehicle?.year}</strong></p>
              ...
              <button onClick={handleSubmit} disabled={loading || !isVerified} className="btn-primary" style={{ borderRadius: '50px', opacity: isVerified ? 1 : 0.5 }}>ยืนยันและออกกรมธรรม์</button>
            </div>
          )}
          ...
          {step === 4 && (
            <div style={{ textAlign: 'center', padding: '3rem 0' }}>
              ...
              <h2>สำเร็จ!</h2>
              <p>เลขกรมธรรม์: {policyResult?.policyNumber}</p>
        ...
        <section className="card" style={{ padding: '1.5rem', background: '#1a1a1a', color: 'white', borderRadius: '16px' }}>
          <h3 style={{ fontSize: '1rem', color: 'white', marginBottom: '1rem' }}>สรุปความคุ้มครอง</h3>
          <p style={{ fontSize: '0.8rem', color: '#888', margin: 0 }}>{selectedPlan.name}</p>
          <p style={{ fontWeight: 'bold' }}>{selectedPlan.planName}</p>
          <div style={{ marginTop: '2rem', paddingTop: '1rem', borderTop: '1px solid #333' }}>
            <p style={{ fontSize: '0.8rem', color: '#888', margin: 0 }}>ยอดโอนจริง ({paymentMode})</p>
